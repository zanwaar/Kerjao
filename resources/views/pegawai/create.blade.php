...
<x-page-header title="Tambah Pegawai" />
    ...
    <form method="POST" action="{{ route('pegawai.store') }}" class="space-y-4">
        ...
        <x-form-field label="NIP" name="nip">
            <input type="text" name="nip" id="nip" value="{{ old('nip') }}"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('nip') border-red-400 @enderror">
        </x-form-field>

        <div class="grid grid-cols-2 gap-4">
            <x-form-field label="Jabatan" name="jabatan" :required="true">
                <input type="text" name="jabatan" id="jabatan" value="{{ old('jabatan') }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('jabatan') border-red-400 @enderror">
            </x-form-field>
            ...
            <x-form-field label="Unit Kerja" name="unit_kerja" :required="true">
                <input type="text" name="unit_kerja" id="unit_kerja" value="{{ old('unit_kerja') }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('unit_kerja') border-red-400 @enderror">
            </x-form-field>
        </div>
        ...
        <x-form-field label="Status" name="status_pegawai" :required="true">
            <select name="status_pegawai" id="status_pegawai" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <option value="nonaktif" @selected(old('status_pegawai') === 'nonaktif')>Non-Aktif</option>
            </select>
        </x-form-field>
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <x-form-field label="WakaTime User Key" name="wakatime_user_key">
                <input type="text" name="wakatime_user_key" id="wakatime_user_key" value="{{ old('wakatime_user_key') }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </x-form-field>
        </div>
