    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                ...
                <h3 class="card-title">Tambah Daily Scrum</h3>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('daily-scrum.store') }}">
                    @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <x-form-field label="Tanggal" name="tanggal" :required="true">
                                <input type="date" name="tanggal" value="{{ old('tanggal', today()->format('Y-m-d')) }}" class="form-control">
                            </x-form-field>
                        </div>
                    </div>
                    ...
                    <x-form-field label="Rencana Kerja Harian" name="rencana_kerja_harian" :required="true">
                        <textarea id="rencana_kerja_harian" name="rencana_kerja_harian" rows="3"
                            ...
                            class="form-control @error('rencana_kerja_harian') is-invalid @enderror">{{ old('rencana_kerja_harian') }}</textarea>
                    ...
                    <x-form-field label="Indikator Capaian" name="indikator_capaian">
                        <textarea id="indikator_capaian" name="indikator_capaian" rows="2"
                            placeholder="Apa indikator keberhasilan hari ini?" class="form-control">{{ old('indikator_capaian') }}</textarea>
                    ...
                    <x-form-field label="Potensi Risiko" name="potensi_risiko">
                        <textarea id="potensi_risiko" name="potensi_risiko" rows="2"
                            placeholder="Ada hambatan yang diantisipasi?" class="form-control">{{ old('potensi_risiko') }}</textarea>
                    ...
                    <x-form-field label="Realisasi" name="realisasi">
                        <textarea id="realisasi" name="realisasi" rows="2"
                            placeholder="Isi setelah pekerjaan selesai..." class="form-control">{{ old('realisasi') }}</textarea>
                    ...
                    <x-form-field label="Rencana Tindak Lanjut" name="rencana_tindak_lanjut">
                        <textarea id="rencana_tindak_lanjut" name="rencana_tindak_lanjut" rows="2" class="form-control">{{ old('rencana_tindak_lanjut') }}</textarea>
