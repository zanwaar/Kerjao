            ...
            <div class="card-body">
                <form method="POST" action="{{ route('kegiatan.update', $kegiatan) }}">
                    ...
                    <x-form-field label="Deskripsi" name="deskripsi">
                        <textarea name="deskripsi" rows="3" class="form-control">{{ old('deskripsi', $kegiatan->deskripsi) }}</textarea>
                    </x-form-field>

                    <x-form-field label="Target Capaian" name="target_capaian">
                        <textarea name="target_capaian" rows="2" class="form-control">{{ old('target_capaian', $kegiatan->target_capaian) }}</textarea>
                    </x-form-field>

                    <div class="row">
                        <div class="col-md-6">
                            <x-form-field label="Waktu Mulai" name="waktu_mulai" :required="true">
                                <input type="date" name="waktu_mulai" value="{{ old('waktu_mulai', $kegiatan->waktu_mulai->format('Y-m-d')) }}" class="form-control">
                            </x-form-field>
                        </div>
                        <div class="col-md-6">
                            <x-form-field label="Waktu Selesai" name="waktu_selesai" :required="true">
                                <input type="date" name="waktu_selesai" value="{{ old('waktu_selesai', $kegiatan->waktu_selesai->format('Y-m-d')) }}" class="form-control">
                            </x-form-field>
                        </div>
                    </div>
